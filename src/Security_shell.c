...
int counterOfCommands(){
	// The function returns the number of the next command that is going to be in the Security commands log
	// Open the commands log
	int fd_commands = open("Commands/Security/Security_Commands.txt", O_RDONLY);
    	if (fd_commands == -1) {
        	perror("open failed");
        	return 1;
    	}
...
int main() {
	/*The main function. It implements a shell which can call 5 programs, any other command will print to the screen Not Supported.
	...
	char buff[256];
	char *args[4]; // Array to store arguments for execve
	int status;
	char counterString[256]="";
	// Creates the Security directory in Commands using mkdir -p
	char * argsc[4];
	int pid = fork();
	if(pid==0){	
		argsc[0]="mkdir";
		argsc[1]="-p";
		argsc[2]="Commands/Security";
		argsc[3]=NULL;
	...
	// Open (or create) the file "Security_commands.txt" inside the "Commands/Security" directory
	int fd_commands = open("Commands/Security/Security_Commands.txt", O_RDWR | O_APPEND | O_CREAT, 0664);
	if (fd_commands == -1) {
		perror("open failed");
		return 1;
	}
	...
	while (1) {
		...
		int pid = fork();
		if (pid == 0) {
		    if(args[0]!=NULL){ // The child process: it will do one of the 6 programs below, any other program(except call onto Esc), will print Not Supported
			    if(strcmp(args[0],"Enc")==0||strcmp(args[0],"AdvEnc")==0||strcmp(args[0],"Dec")==0||strcmp(args[0],"AdvDec")==0){
			    	execve(args[0],args,NULL);
			    }
			    else if(strcmp(args[0],"History")==0){
			    // Calling History program is a bit different, since history prints the Security_commands it needs to be in the log before the print,
			    // so we entered History in the log before calling it.
		    		...
		    		if(write(fd_commands,"\n",strlen("\n"))==-1)
		    			perror("Writing failed");
		    		// Also, since we have only 1 History program, we must specify which log to print, in this case since we are in the Security shell it will print
		    		// the log of the Security shell
			    	args[arg_count++]= "Commands/Security/Security_Commands.txt";
			    	args[arg_count] = NULL;
			    	execve(args[0],args,NULL);
			    }
			    else{
		    ...
		    else if(status==0 && strcmp(args[0],"History") != 0){ // Adds the rest of the successfull programs
	...
	// Closes the Security commands log and exits the shell
	close(fd_commands);
	return 0;
}
