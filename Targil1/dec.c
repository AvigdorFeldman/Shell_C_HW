...
int main(int argc , char * argv[]){
	/*The main function. The program is called from the Security shell, it gets a name of a file and a character to decrypt the recieved file using XOR,
	...
	// Checks if the path of the file is in the Encrypt_File directory directory, if its not it will print "Decryption option not Supported\n" and exit the program with -2
	char path[256] = "Encryption_File/";
	int fd_file1;
    	strcat(path, argv[1]);
    	if(fd_file1=open(path,O_RDONLY)==-1){
    		fprintf(stdout, "Decryption option not Supported\n"); return -2;
    	}
    	close(fd_file1);
	...
	int fd_file;
	unsigned char ch;
	// Opens the file to decryption
	if((fd_file = open(path,O_RDWR,0664))==-1){
	...
	// The decryption of the file using the recieved character and XOR
	for(int i=0;i<size;i++){
		if(read(fd_file,&ch,1)==-1){// Reads one character
			perror("Failed Read");close(fd_file); return -4;		
		}
		if(lseek(fd_file, -1, SEEK_CUR)==-1){
	...
	close(fd_file);
	// Moves the encrypted file to the current directory
	int pid,status;
	for(int i=0;i<4;i++)
		args[i]=NULL;
	args[0] = "mv";
	args[1] = path;
	args[2] = "."; // The current directory
	args[3]=NULL;
	pid = fork();
	if(pid == 0){// The child process: moves the file to the current directory using execvp on command mv
		execvp("mv",args);
		perror("mv");
		exit(1);
	}
